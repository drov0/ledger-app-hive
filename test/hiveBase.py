...
class Transaction:
    ...
    def parse(json):
        ...
        tx.ref_block_num = struct.pack('<H', body['ref_block_num']) # good
        tx.ref_block_prefix = struct.pack('<I', body['ref_block_prefix']) # good

        timeformat = "%Y-%m-%dT%H:%M:%S%Z"
        tx.expiration = struct.pack("<I", timegm(time.strptime((body["expiration"] + "UTC"), timeformat))) # good
        ...
        tx.extensions_size = struct.pack("<h", len(body.get('extensions', [])))
